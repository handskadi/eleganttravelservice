...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    ...
    alternates: {
      canonical: "https://www.eleganttravelservice.com/careers",
      languages: {
        en: "https://www.eleganttravelservice.com/careers",
        fr: "https://www.eleganttravelservice.com/fr/careers",
        ar: "https://www.eleganttravelservice.com/ar/careers",
        es: "https://www.eleganttravelservice.com/es/careers",
        de: "https://www.eleganttravelservice.com/de/careers",
        it: "https://www.eleganttravelservice.com/it/careers",
      },
    },
  };
}
    ...
    <div className="min-h-screen bg-slate-50 pt-20" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 py-20 px-6">
        <div className="max-w-screen-xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-4">{t("heroHeading")}</h1>
          <p className="text-slate-400 text-lg max-w-xl mx-auto">{t("heroSubtext")}</p>
        </div>
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12 py-16">
        ...
        {/* Jobs */}
        <div className="mb-6">
          <h2 className="text-2xl font-extrabold text-slate-800 mb-1">{t("openPositions")}</h2>
          <p className="text-sm text-slate-500">{t("rolesAvailable", { count: jobs.length })}</p>
        </div>
          ...
          {jobs.map(job => (
            <div key={job.id} className="bg-white rounded-2xl shadow-sm hover:shadow-md border border-slate-100 p-6 transition group">
              <div className="flex flex-col md:flex-row md:items-start gap-4">
                ...
                >
                  {t("applyNow")} <FaArrowRight className="w-3.5 h-3.5" />
                </Link>
              </div>
            </div>
          ))}
        ...
        <div className="mt-10 bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-8 text-center">
          <h3 className="text-xl font-extrabold text-white mb-2">{t("noRoleLabel")}</h3>
          <p className="text-slate-400 text-sm mb-4 max-w-md mx-auto">{t("noRoleText")}</p>
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold px-6 py-3 rounded-xl transition"
          >
            {t("noRoleBtn")} <FaArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </div>
    </div>
