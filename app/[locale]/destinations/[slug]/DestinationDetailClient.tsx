...
import { destFacets } from "@/data/destination-facets";
...
function RelatedDestCard({ dest }: { dest: DestinationData }) {
  ...
  const relContent = destContent[dest.slug]?.[locale] ?? destContent[dest.slug]?.["en"];
  const localeSlug = destSlugs[dest.slug]?.[locale] ?? dest.slug;
  return (
    <LocaleLink
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 flex items-end justify-between">
        <div>
          <p className="font-extrabold text-white text-lg leading-tight">{relContent?.name ?? dest.name}</p>
          <p className="text-amber-300 text-xs mt-0.5">{relContent?.subtitle ?? dest.subtitle}</p>
          <p className="text-white/50 text-[10px] mt-1 uppercase tracking-widest">
            {relContent?.region ?? dest.region}
          </p>
        </div>
    ...
    <article className="tour-card bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 flex flex-col h-full group">
      ...
      <div className="flex flex-col flex-1 p-4 gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-slate-400">({tour.reviews.toLocaleString()} {t("reviews")})</span>
        </div>
        ...
        <div className="flex items-center justify-between pt-3 mt-1 border-t border-slate-100 gap-2">
          <div className="leading-none">
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-[11px] text-slate-400 font-normal">/ {t("perPerson")}</span>
            </div>
            {tour.originalPrice && (
...
export default function DestinationDetailClient({ slug }: { slug: string }) {
  ...
  const locale = useLocale();
  const dp = useTranslations("dp");

  const dest = destinationsData.find(d => d.slug === slug);
  const content = dest ? (destContent[dest.slug]?.[locale] ?? destContent[dest.slug]?.["en"]) : null;
  const facets = dest ? (destFacets[dest.slug]?.[locale] ?? destFacets[dest.slug]?.["en"]) : null;
  ...
  if (!dest) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 px-6 pt-24 pb-16 text-center">
        ...
        <h1 className="text-2xl font-extrabold text-slate-800 mb-2">{dp("notFoundTitle")}</h1>
  ...
  const displayName     = content?.name       ?? dest.name;
  const displaySubtitle = content?.subtitle   ?? dest.subtitle;
  const displayRegion   = content?.region     ?? dest.region;
  const displayAbout    = content?.about      ?? dest.about;
  const displayMonths   = content?.bestMonths ?? dest.bestMonths;
  const displayGetting  = content?.gettingThere ?? dest.gettingThere;
  const displayTips     = content?.localTips  ?? dest.localTips;

  const displayHighlights = content?.highlights
    ? content.highlights.map((h, i) => ({ ...h, icon: dest.highlights[i]?.icon ?? "FaStar" }))
    : dest.highlights;
  ...
  // ── Stat pills data ────────────────────────────────────────────────────────
  const facetLang     = facets?.language ?? dest.language;
  const facetCurrency = facets?.currency ?? dest.currency;
  const facetAirport  = facets?.airport  ?? dest.airport;
  const facetCountry  = facets?.country  ?? dest.country;

  const statPills = [
    { label: dp("pillBestMonths"), value: displayMonths.slice(0, 3).join(", ") },
    { label: dp("pillLanguage"),   value: facetLang.split(/[,،]/)[0].trim() },
    { label: dp("pillCurrency"),   value: facetCurrency },
    { label: dp("pillTimezone"),   value: dest.timezone },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="relative h-[600px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-between px-6 md:px-10 lg:px-16 pt-28 pb-10 max-w-screen-xl mx-auto left-0 right-0">
          ...
          <nav className="flex items-center gap-2 text-xs text-white/60" aria-label="Breadcrumb">
            <LocaleLink href="/" className="hover:text-amber-300 transition-colors">{dp("breadHome")}</LocaleLink>
            <span className="text-white/30">/</span>
            <LocaleLink href="/destinations" className="hover:text-amber-300 transition-colors">{dp("breadDestinations")}</LocaleLink>
            <span className="text-white/30">/</span>
        ...
        <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12 py-16 md:py-20">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-16">
            ...
            <div className="lg:col-span-2">
              <span className="section-label">{dp("aboutLabel", { name: displayName })}</span>
              <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mt-1 mb-6 leading-tight">
                {dp("discoverTitle", { name: displayName })}
              </h2>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-lg p-6">
                ...
                <div className="flex flex-col gap-4">
                  {[
                    { Icon: FaSun,          label: dp("summerTemp"),  value: dest.avgTempSummer },
                    { Icon: FaSnowflake,    label: dp("winterTemp"),  value: dest.avgTempWinter },
                    { Icon: FaPlane,        label: dp("airport"),     value: facetAirport },
                    { Icon: FaLanguage,     label: dp("languages"),   value: facetLang },
                    { Icon: FaMoneyBillWave,label: dp("currency"),    value: facetCurrency },
                    { Icon: FaClock,        label: dp("timezone"),    value: dest.timezone },
      ...
      >
        <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12">
          <div className="text-center mb-10">
            <span className="section-label">{dp("mustSeeLabel")}</span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mt-1">
              {dp("topHighlights")}
            </h2>
            <p className="text-slate-500 mt-3 max-w-xl mx-auto text-sm">
              {dp("highlightsDesc", { name: displayName })}
            </p>
          </div>
      ...
      {/* ════════════════════════════════════════════════════════════════════
          4. THINGS TO DO
      ════════════════════════════════════════════════════════════════════ */}
      {displayThingsToDo.length > 0 && (
        <section className="bg-white py-16 md:py-20">
          <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12">
            <div className="flex items-end justify-between mb-10 gap-4">
              <div>
                <span className="section-label">{dp("curatedLabel")}</span>
                <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mt-1">
                  {dp("topExperiences")}
                </h2>
              </div>
      ...
      {destTours.length > 0 && (
        ...
        >
          <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12">
            <div className="flex items-end justify-between mb-10 gap-4">
              <div>
                <span className="section-label">{dp("bookYourTripLabel")}</span>
                <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mt-1">
                  {dp("toursInDest", { count: destTours.length, name: displayName })}
                </h2>
              </div>
              <Link
                ...
                className="flex items-center gap-1.5 text-sm font-semibold text-amber-600 hover:text-amber-700 transition-colors whitespace-nowrap shrink-0"
              >
                {dp("browseAll")} <HiArrowRight className="w-4 h-4" />
              </Link>
            </div>
      ...
      <section className="bg-white py-16 md:py-20">
        <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12">
          <div className="text-center mb-10">
            <span className="section-label">{dp("insiderLabel")}</span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mt-1">
              {dp("travelTipsTitle")}
            </h2>
          </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-md p-7">
              ...
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-700">{facetAirport}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {relatedDests.length > 0 && (
        ...
        >
          <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12">
            <div className="flex items-end justify-between mb-10 gap-4">
              <div>
                <span className="section-label">{dp("exploreMoreLabel")}</span>
                <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mt-1">
                  {dp("youMightAlsoLike")}
                </h2>
              </div>
              <Link
                ...
                className="flex items-center gap-1.5 text-sm font-semibold text-amber-600 hover:text-amber-700 transition-colors whitespace-nowrap shrink-0"
              >
                {dp("allDestinations")} <HiArrowRight className="w-4 h-4" />
              </Link>
            </div>
