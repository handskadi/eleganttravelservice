      ...
      <div className="relative h-52 overflow-hidden bg-slate-100">
        ...
        {tour.badge && (
          <span className={`absolute top-3 right-3 z-10 text-[10px] font-bold px-2.5 py-1 rounded-full shadow-sm ${BADGE_STYLES[tour.badgeColor ?? "slate"]}`}>
            {tour.badge === "Featured" ? t("featured") : tour.badge}
          </span>
        )}
        ...
        <div className="flex items-center justify-between pt-3 mt-1 border-t border-slate-100 gap-2">
          ...
          <div className="leading-none">
            <div className="flex items-baseline gap-1">
              <span className="text-[10px] text-slate-400 font-normal">{tc("from")}</span>
              <span className="text-xl font-extrabold text-slate-900 tabular-nums">${tour.price}</span>
              <span className="text-[11px] text-slate-400 font-normal">/ {tc("perPerson")}</span>
            </div>
            {tour.originalPrice && (
...
const CITY_CONFIGS_BASE: {
  ...
  { label: "Marrakech",   labelKey: "cityMarrakechName",   icon: FaStar,       colour: "text-amber-600",   bg: "bg-amber-50",   border: "border-amber-100",   taglineKey: "cityMarrakechTagline",   href: "/tours/from-marrakech" },
  { label: "Casablanca",  labelKey: "cityCasablancaName",  icon: FaBuilding,   colour: "text-slate-600",   bg: "bg-slate-50",   border: "border-slate-200",   taglineKey: "cityCasablancaTagline",  href: "/tours/from-casablanca" },
  { label: "Fes",         labelKey: "cityFesName",         icon: FaUniversity, colour: "text-purple-600",  bg: "bg-purple-50",  border: "border-purple-100",  taglineKey: "cityFesTagline",         href: "/tours/from-fes" },
  { label: "Tangier",     labelKey: "cityTangierName",     icon: FaAnchor,     colour: "text-blue-600",    bg: "bg-blue-50",    border: "border-blue-100",    taglineKey: "cityTangierTagline",     href: "/tours/from-tangier" },
  { label: "Agadir",      labelKey: "cityAgadirName",      icon: FaUmbrella,   colour: "text-cyan-600",    bg: "bg-cyan-50",    border: "border-cyan-100",    taglineKey: "cityAgadirTagline",      href: "/tours/from-agadir" },
  { label: "Grand Tours", labelKey: "cityGrandToursName",  icon: FaGlobe,      colour: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-100", taglineKey: "cityGrandToursTagline" },
];
...
export default function ToursClient() {
  ...
  const CITY_CONFIGS = CITY_CONFIGS_BASE.map(c => ({
    ...c,
    tagline:      t(c.taglineKey  as Parameters<typeof t>[0]),
    displayLabel: t(c.labelKey   as Parameters<typeof t>[0]),
  }));
      ...
      >
        ...
        <div className="relative max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12">
          ...
          <nav className="flex items-center gap-2 text-xs text-slate-400 mb-6" aria-label="Breadcrumb">
            <Link href="/" className="hover:text-amber-400 transition-colors">{t("breadcrumbHome")}</Link>
            <span className="text-slate-600">/</span>
            <span className="text-amber-400 font-medium">{t("breadcrumbTours")}</span>
          </nav>

          {/* Heading */}
          <div className="max-w-2xl">
            <span className="section-label mb-4">
              {multiDayTours.length}+ {t("heroLabel")}
            </span>
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3 max-w-2xl">
            ...
            <div className="relative">
              ...
              >
                <option value="featured"   className="text-slate-900 bg-white">{t("sortFeatured")}</option>
                <option value="price-asc"  className="text-slate-900 bg-white">{t("sortPriceAsc")}</option>
                <option value="price-desc" className="text-slate-900 bg-white">{t("sortPriceDesc")}</option>
                <option value="rating"     className="text-slate-900 bg-white">{t("sortRating")}</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12">
          ...
          <div className="pt-10 pb-6">
            <div className="text-center mb-8">
              <span className="section-label">{t("citySectionLabel")}</span>
              <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 mt-2">
                {t("citySectionHeading")}
              </h2>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {CITY_CONFIGS.map(city => {
                ...
                return city.href ? (
                  <Link key={city.label} href={city.href as `/tours/${string}`} className={cardClass}>
                    {inner}
                  </Link>
                ) : (
                  <button
          ...
          {filtered.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-28 gap-4 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-800">{t("noToursHeading")}</h3>
