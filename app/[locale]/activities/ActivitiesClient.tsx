...
export default function ActivitiesClient() {
  ...
  // Category display translations (labels used as filter keys stay in English)
  const catDisplay: Record<string, { label: string; tagline: string }> = {
    "Day Trips":       { label: t("catDayTrips"),      tagline: t("catDayTripsTagline") },
    "Adventure":       { label: t("catAdventure"),     tagline: t("catAdventureTagline") },
    "Food & Culture":  { label: t("catFoodCulture"),   tagline: t("catFoodCultureTagline") },
    "Wellness":        { label: t("catWellness"),      tagline: t("catWellnessTagline") },
    "Nature & Desert": { label: t("catNatureDesert"),  tagline: t("catNatureDesertTagline") },
  };

  const BENEFITS_I18N = [
    { icon: FaShieldAlt, title: t("benefit1Title"), desc: t("benefit1Desc"), colour: "text-emerald-600", bg: "bg-emerald-50" },
    { icon: FaHeadset,   title: t("benefit2Title"), desc: t("benefit2Desc"), colour: "text-blue-600",    bg: "bg-blue-50"    },
    { icon: FaAward,     title: t("benefit3Title"), desc: t("benefit3Desc"), colour: "text-amber-600",   bg: "bg-amber-50"   },
    { icon: FaRoute,     title: t("benefit4Title"), desc: t("benefit4Desc"), colour: "text-purple-600",  bg: "bg-purple-50"  },
  ];
        ...
        >
          ...
          <div className="relative max-w-screen-xl mx-auto">
            <nav className="flex items-center gap-2 text-xs text-slate-400 mb-6" aria-label="Breadcrumb">
              ...
              <span className="text-amber-400 font-medium">{t("heroHeading")}</span>
            </nav>

            <div className="max-w-2xl">
              <span className="section-label mb-4 inline-block">{t("heroSectionLabel")}</span>
              ...
              <div className="inline-flex items-center gap-2.5 px-4 py-2.5 bg-white/10 border border-white/20 rounded-xl text-sm text-white/80 backdrop-blur-sm mb-8">
                ...
                <Link href="/tours" className="text-amber-400 font-semibold hover:text-amber-300 transition flex items-center gap-1">
                  {t("browseToursLink")} <FaArrowRight className="w-3 h-3" />
                </Link>
              </div>
            </div>
            ...
            <div className="max-w-xl relative mb-8">
              <FaSearch className={`absolute ${isRTL ? "right-4" : "left-4"} top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none`} />
              <input
                type="text"
                value={searchQuery}
                ...
                dir={isRTL ? "rtl" : "ltr"}
                className={`w-full ${isRTL ? "pr-11 pl-4" : "pl-11 pr-4"} py-3.5 rounded-2xl bg-white/10 backdrop-blur border border-white/15 text-white placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 transition`}
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery("")} className={`absolute ${isRTL ? "left-4" : "right-4"} top-1/2 -translate-y-1/2 text-slate-400 hover:text-white text-xs font-semibold`}>
                  {t("clearFilters")}
                </button>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-3 text-xs text-slate-400">
              {[`${allActivities.length} ${t("trust1")}`, t("trust2"), t("trust3"), t("trust4")].map(tag => (
      ...
      <div className="max-w-screen-xl mx-auto px-6 md:px-8 lg:px-12 py-12">
        ...
        <section className="mb-12">
          <div className="text-center mb-8">
            <span className="section-label">{t("categorySectionLabel")}</span>
            <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 mt-2">
              {t("categorySectionHeading")}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {CATEGORIES.map(cat => {
              ...
              const count = allActivities.filter(a => CATEGORY_IDS[cat.label as Exclude<CategoryLabel, "All">].includes(a.id)).length;
              const display = catDisplay[cat.label] ?? { label: cat.label, tagline: cat.tagline };
              return (
                <button
                  key={cat.label}
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-extrabold text-slate-900">
              {activeCategory === "All" ? t("resultsAll") : catDisplay[activeCategory]?.label ?? activeCategory}
            </h2>
            <p className="text-sm text-slate-500 mt-0.5">
              {filtered.length} {t("trust1").toLowerCase()}
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-20 mb-16">
            ...
            <p className="text-slate-500 text-lg font-medium mb-1">{t("emptyHeading")}</p>
            <p className="text-slate-400 text-sm mb-4">{t("emptyText")}</p>
            <button
              onClick={() => { setActiveCategory("All"); setSearchQuery(""); }}
        ...
        <section className="mb-14 bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl overflow-hidden p-8 md:p-10 flex flex-col md:flex-row items-center gap-6 border border-white/5">
          <div className="flex-1">
            <span className="text-amber-400 text-xs font-bold uppercase tracking-widest">{t("crossPromoLabel")}</span>
            <h3 className="text-2xl font-extrabold text-white mt-2 mb-2">{t("crossPromoHeading")}</h3>
            <p className="text-slate-400 text-sm leading-relaxed max-w-lg">{t("crossPromoText")}</p>
          </div>
          <Link
            href="/tours"
        ...
        <section className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-3xl overflow-hidden px-8 py-14 mb-14 relative">
          ...
          <div className="relative text-center mb-10">
            <span className="section-label inline-block mb-3 bg-amber-500/15 border-amber-500/30 text-amber-400">{t("whyBookLabel")}</span>
            <h2 className="text-2xl md:text-3xl font-extrabold text-white mt-2">{t("whyBookHeading")}</h2>
            <p className="text-slate-400 text-base mt-2 max-w-xl mx-auto">{t("whyBookSubtitle")}</p>
          </div>
        ...
        <section className="mb-14">
          <div className="text-center mb-8">
            <span className="section-label">{t("faqSectionLabel")}</span>
            <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 mt-2">{t("faqHeading")}</h2>
          </div>
          <div className="max-w-3xl mx-auto space-y-3">
            {FAQS_I18N.map(faq => <FaqItem key={faq.q} q={faq.q} a={faq.a} />)}
          </div>
        </section>
        ...
        <section className="relative bg-gradient-to-br from-amber-500 to-amber-600 rounded-3xl overflow-hidden px-8 py-14 text-center">
          ...
          <div className="relative">
            <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-4">{t("ctaHeading")}</h2>
            <p className="text-amber-100 text-base max-w-xl mx-auto mb-8">{t("ctaText")}</p>
            <div className="flex flex-wrap gap-4 justify-center">
              <Link href="/contact"
                ...
                {t("startPlanning")} <HiArrowRight className="w-4 h-4" />
              </Link>
              <Link href="/tours"
                ...
                {t("browseAllTours")} <FaArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
          </div>
        </section>
